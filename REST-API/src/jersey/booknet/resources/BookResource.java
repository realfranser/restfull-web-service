...
public class BookResource {
  ...
  /*
   * GET call is not necesary in the first implementation // Web API
   * 
   * @Path("{isbn}")
   * 
   * @GET
   ...
   * Response getBook(@PathParam("isbn") int isbn) { Response res; Book book;
   * if(BookDao.getInstance().getModel().containsKey(isbn)) { book=
   * BookDao.getInstance().getModel().get(isbn); res = Response.ok(book).build();
   * } else { //throw new RuntimeException("Get: Tarea con isbn " +
   * Integer.toString(isbn) + " no encontrada"); res =
   * Response.status(Response.Status.NOT_FOUND).build(); } return res; }
   * 
   ...
   * @GET
   * 
   * @Produces(MediaType.TEXT_XML) public Response getTodoHTML(@PathParam("book")
   * int isbn) { Response res; Book book;
   * if(BookDao.getInstance().getModel().containsKey(isbn)) { book =
   * BookDao.getInstance().getModel().get(isbn); res = Response.ok(book).build();
   * } else { //throw new RuntimeException("Get: Tarea con isbn " +
   * Integer.toString(isbn) + " no encontrada"); res =
  ...
  public Response deleteBook(@PathParam("isbn") int isbn) {
    ...
    if (BookDao.getInstance().getModel().containsKey(isbn)) {
      ...
      // throw new RuntimeException("Delete: Tarea con isbn " + Integer.toString(isbn)
      // + " no encontrada");
