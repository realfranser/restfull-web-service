...
public class restRecursos {
	...
	public ArrayList<jersey.booknet.model.User> getUsersWithName(String user_name) { // returns all users with a given
																						// name
		if (conn == null) {
			connect();
		}
	...
	public ResultSet getFullUserInfo(int user_id) {
		 ...
		 * prepStmt.executeQuery(); conn.commit(); rs.next(); int isbn = rs.getInt(1);
		 * String book_name = rs.getString(2); String authors_name = rs.getString(3);
		 * String category = rs.getString(4);
		 */
		User basic_info = getUser(user_id);
		Book book = getLastReadBooks(user_id, null, 0, 1);
